...
# Función ultra-robusta para obtener el último tiempo de arranque
get_last_boot_time() {
    echo "<div class='card'><h2>⏱️ Último Tiempo de Arranque</h2>"
    echo "<table>"
    
    # Método 1: sysctl (el más confiable)
    if boot_time=$(sysctl -n kern.boottime 2>/dev/null); then
        boot_timestamp=$(echo "$boot_time" | awk '{print $4}' | tr -d ',')
        boot_date=$(date -jf "%s" "$boot_timestamp" "+%Y-%m-%d %H:%M:%S")
        echo "<tr><th>Último arranque:</th><td>$boot_date</td></tr>"
        echo "<tr><th>Fuente:</th><td>sysctl kern.boottime</td></tr>"
        echo "</table></div>"
        return
    fi

    # Método 2: last reboot (alternativa común)
    if last_reboot=$(last reboot | head -1 2>/dev/null); then
        if [[ -n "$last_reboot" ]]; then
            reboot_date=$(echo "$last_reboot" | awk '{print $5" "$6" "$7" "$8}')
            echo "<tr><th>Último reinicio:</th><td>$reboot_date</td></tr>"
            echo "<tr><th>Fuente:</th><td>last reboot</td></tr>"
            echo "</table></div>"
            return
        fi
    fi

    # Método 3: system_profiler (para fecha de inicio)
    if boot_info=$(system_profiler SPSoftwareDataType 2>/dev/null | grep "Time since Boot"); then
        boot_time=$(echo "$boot_info" | awk -F': ' '{print $2}')
        echo "<tr><th>Tiempo desde arranque:</th><td>$boot_time</td></tr>"
        echo "<tr><th>Fuente:</th><td>system_profiler</td></tr>"
        echo "</table></div>"
        return
    fi

    # Método 4: uptime (último recurso)
    if uptime_info=$(uptime 2>/dev/null); then
        echo "<tr><th>Uptime:</th><td>$uptime_info</td></tr>"
        echo "<tr><th>Fuente:</th><td>uptime</td></tr>"
        echo "</table></div>"
        return
    fi

    # Si todo falla
    echo "<tr><td>No se pudo determinar el tiempo de arranque</td></tr>"
    echo "<tr><td><small>Nota: Esto puede ser normal en algunas configuraciones</small></td></tr>"
    echo "</table></div>"
}
...
# Añadir último tiempo de arranque
get_last_boot_time >> "$HTML_REPORT"
